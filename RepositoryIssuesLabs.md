...
**Note:** This command will only update packages in repositories with an ID containing the string ‘microsoft’. This will cause the RHUI package to be updated. It will not otherwise update the VM software to the latest available versions. 
...
When you have an Azure VM hosted behind a Standard Internal Load Balancer, the Azure VM will not be able to communicate to the internet, unless the load balancer is external facing, or if the Azure VM is associated to a Public IP of standard SKU. For this lab, the suggestion is to add a public IP address to the VM.
...
The public IP address must be attached directly to the network interface on the VM. You may filter traffic to the VM for security reasons using Network Security Groups. 
